...
struct AeronConfig {
    ...
    ygg::config::StreamConfig control{"aeron:ipc", 2001};       // Fenrir → Huginn subscription control
    ygg::config::StreamConfig data{"aeron:ipc", 2002};          // Huginn → Fenrir market data
    ygg::config::StreamConfig ack_hb{"aeron:ipc", 2003};        // Huginn → Fenrir acks + heartbeats
    ygg::config::StreamConfig funding_rate{"aeron:ipc", 1005};  // Huginn → Fenrir funding rates
};
