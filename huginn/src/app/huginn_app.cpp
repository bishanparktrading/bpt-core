...
#include "huginn/adapter/deribit/deribit_adapter.h"
...
HuginnApp::HuginnApp(config::Settings cfg, std::shared_ptr<aeron::Aeron> aeron)
    : cfg_(std::move(cfg)),
      aeron_(aeron),
    ...
    for (const auto& a_cfg : cfg_.adapters) {
        ...
        // lifetime of HuginnApp (metrics_ is a member).
