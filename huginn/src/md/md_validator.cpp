#include "huginn/md/md_validator.h"
...
ValidationResult MdValidator::validate(const MdBbo& bbo) {
    if (bbo.bid_price <= 0.0 || bbo.ask_price <= 0.0) {
        if (should_log(drop_count_, bbo.instrument_id))
            spdlog::warn("[MdValidator] BBO invalid prices: bid={} ask={} id={}",
                         bbo.bid_price,
                         bbo.ask_price,
                         bbo.instrument_id);
    ...
    if (bbo.bid_qty <= 0.0 || bbo.ask_qty <= 0.0) {
        if (should_log(drop_count_, bbo.instrument_id))
            spdlog::warn("[MdValidator] BBO invalid quantities: bid_qty={} ask_qty={} id={}",
                         bbo.bid_qty,
                         bbo.ask_qty,
                         bbo.instrument_id);
        return ValidationResult::DROP;
    ...
    if (bbo.ask_price <= bbo.bid_price) {
        if (should_log(drop_count_, bbo.instrument_id))
            spdlog::warn("[MdValidator] BBO crossed: bid={} ask={} id={}",
                         bbo.bid_price,
                         bbo.ask_price,
                         bbo.instrument_id);
        return ValidationResult::DROP;
    ...
    if (it != last_mid_.end()) {
        ...
        if (deviation > max_deviation_ratio_) {
            if (should_log(drop_count_, bbo.instrument_id))
                spdlog::warn("[MdValidator] BBO price deviation {:.2f}% > {:.2f}%: mid={} last={} id={}",
                             deviation * 100.0,
...
ValidationResult MdValidator::validate(const MdTrade& trade) {
    if (trade.price <= 0.0) {
        if (should_log(drop_count_, trade.instrument_id))
            spdlog::warn("[MdValidator] Trade invalid price={} id={}", trade.price, trade.instrument_id);
        return ValidationResult::DROP;
    }
    if (trade.qty <= 0.0) {
        if (should_log(drop_count_, trade.instrument_id))
            spdlog::warn("[MdValidator] Trade invalid qty={} id={}", trade.qty, trade.instrument_id);
        return ValidationResult::DROP;
    }
...
ValidationResult MdValidator::validate(const MdOrderBook& book) {
    if (book.bids.empty() || book.asks.empty()) {
        if (should_log(drop_count_, book.instrument_id))
            spdlog::warn("[MdValidator] OrderBook empty side id={}", book.instrument_id);
        return ValidationResult::DROP;
    }
    ...
    if (best_bid <= 0.0 || best_ask <= 0.0) {
        if (should_log(drop_count_, book.instrument_id))
            spdlog::warn("[MdValidator] OrderBook invalid prices: bid={} ask={} id={}",
                         best_bid,
                         best_ask,
                         book.instrument_id);
        return ValidationResult::DROP;
    ...
    if (best_ask <= best_bid) {
        if (should_log(drop_count_, book.instrument_id))
            spdlog::warn("[MdValidator] OrderBook crossed: bid={} ask={} id={}",
                         best_bid,
                         best_ask,
                         book.instrument_id);
        return ValidationResult::DROP;
    ...
    for (std::size_t i = 1; i < book.bids.size(); ++i) {
        if (book.bids[i].first >= book.bids[i - 1].first) {
            if (should_log(drop_count_, book.instrument_id))
                spdlog::warn("[MdValidator] OrderBook bids not descending id={}", book.instrument_id);
            return ValidationResult::DROP;
        }
    }
    for (std::size_t i = 1; i < book.asks.size(); ++i) {
        if (book.asks[i].first <= book.asks[i - 1].first) {
            if (should_log(drop_count_, book.instrument_id))
                spdlog::warn("[MdValidator] OrderBook asks not ascending id={}", book.instrument_id);
            return ValidationResult::DROP;
        }
    }
