...
#ifndef _BIFROST_PROTOCOL_SURTRHEARTBEAT_CXX_H_
#define _BIFROST_PROTOCOL_SURTRHEARTBEAT_CXX_H_
...
public:
    ...
    SurtrHeartbeat() = default;

    SurtrHeartbeat(char* buffer,
    ...
    SurtrHeartbeat(char* buffer, const std::uint64_t bufferLength)
        : SurtrHeartbeat(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion()) {}

    SurtrHeartbeat(char* buffer,
                   const std::uint64_t bufferLength,
                   const std::uint64_t actingBlockLength,
                   const std::uint64_t actingVersion)
        : SurtrHeartbeat(buffer, 0, bufferLength, actingBlockLength, actingVersion) {}
    ...
    SurtrHeartbeat& wrapForEncode(char* buffer, const std::uint64_t offset, const std::uint64_t bufferLength) {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    SurtrHeartbeat& wrapAndApplyHeader(char* buffer, const std::uint64_t offset, const std::uint64_t bufferLength) {
    ...
    SurtrHeartbeat& wrapForDecode(char* buffer,
                                  const std::uint64_t offset,
                                  const std::uint64_t actingBlockLength,
                                  const std::uint64_t actingVersion,
                                  const std::uint64_t bufferLength) {
    ...
    SBE_NODISCARD std::uint64_t decodeLength() const {
        SurtrHeartbeat skipper(m_buffer, m_offset, m_bufferLength, sbeBlockLength(), m_actingVersion);
        skipper.skip();
    ...
    SurtrHeartbeat& timestampNs(const std::uint64_t value) SBE_NOEXCEPT {
    ...
    SurtrHeartbeat& seqNum(const std::uint64_t value) SBE_NOEXCEPT {
        std::uint64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    friend std::basic_ostream<CharT, Traits>& operator<<(std::basic_ostream<CharT, Traits>& builder,
                                                         const SurtrHeartbeat& _writer) {
        SurtrHeartbeat writer(_writer.m_buffer,
                              _writer.m_offset,
