...
#ifndef _BIFROST_PROTOCOL_SURTRREADY_CXX_H_
#define _BIFROST_PROTOCOL_SURTRREADY_CXX_H_
...
public:
    ...
    SurtrReady(char* buffer, const std::uint64_t bufferLength)
        : SurtrReady(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion()) {}

    SurtrReady(char* buffer,
               const std::uint64_t bufferLength,
               const std::uint64_t actingBlockLength,
               const std::uint64_t actingVersion)
        : SurtrReady(buffer, 0, bufferLength, actingBlockLength, actingVersion) {}
    ...
    SurtrReady& wrapForEncode(char* buffer, const std::uint64_t offset, const std::uint64_t bufferLength) {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    SurtrReady& wrapAndApplyHeader(char* buffer, const std::uint64_t offset, const std::uint64_t bufferLength) {
    ...
    SurtrReady& wrapForDecode(char* buffer,
                              const std::uint64_t offset,
                              const std::uint64_t actingBlockLength,
                              const std::uint64_t actingVersion,
                              const std::uint64_t bufferLength) {
    ...
    SBE_NODISCARD std::uint64_t decodeLength() const {
        SurtrReady skipper(m_buffer, m_offset, m_bufferLength, sbeBlockLength(), m_actingVersion);
        skipper.skip();
    ...
    SurtrReady& timestampNs(const std::uint64_t value) SBE_NOEXCEPT {
    ...
    SurtrReady& exchangesLoaded(const std::uint8_t value) SBE_NOEXCEPT {
    ...
    SurtrReady& underlyingCount(const std::uint16_t value) SBE_NOEXCEPT {
    ...
    SurtrReady& pointCount(const std::uint32_t value) SBE_NOEXCEPT {
    ...
    friend std::basic_ostream<CharT, Traits>& operator<<(std::basic_ostream<CharT, Traits>& builder,
                                                         const SurtrReady& _writer) {
        SurtrReady writer(_writer.m_buffer,
                          _writer.m_offset,
