...
RadarBus RadarAeronBus::build(std::shared_ptr<::aeron::Aeron> aeron, const config::Settings& settings) {
    ...
    bus.trade_sub =
        std::make_unique<aeron::MdTradeSubscriber>(aeron, settings.md_data.channel, settings.md_data.stream_id);
    bus.bbo_sub =
        std::make_unique<aeron::MdMarketDataSubscriber>(aeron, settings.md_data.channel, settings.md_data.stream_id);
