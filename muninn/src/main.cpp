...
int main(int argc, char** argv) {
    ...
    ygg::log::info("Starting Muninn Reference Data Service...");
    ...
    for (const auto& a_cfg : settings.adapters) {
        if (a_cfg.secret_name.empty()) {
            ygg::log::warn("[Muninn] No secret_name for {} — adapter will have empty credentials", a_cfg.exchange);
        ...
        try {
            ...
            ygg::log::info("[Muninn] Loaded credentials for {}", a_cfg.exchange);
        } catch (const std::exception& e) {
            ygg::log::error("[Muninn] Failed to load credentials for {}: {}", a_cfg.exchange, e.what());
