...
struct InstrumentMappingConfig {
    ...
    // S3 source — if bucket is non-empty, Muninn fetches the per-exchange mapping
    ...
    // Only the exchanges listed in keys are fetched — omit exchanges Muninn doesn't need.
    struct S3 {
        std::string bucket;
...
struct Settings {
    ...
    std::string media_driver_dir;        // Path to bifrost-fabric's Aeron driver dir
    ...
    ygg::config::StreamConfig muninn_status;
