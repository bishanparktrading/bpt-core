...
public:
    ...
    virtual bifrost::protocol::ExchangeId::Value exchange_id() const = 0;

    // Fired for each instrument ADD/MODIFY/REMOVE.
    std::function<void(const refdata::Instrument&, bifrost::protocol::DeltaUpdateType::Value, uint64_t collected_ts_ns)>
        on_instrument_delta;
