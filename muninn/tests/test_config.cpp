...
protected:
    void SetUp() override {
        std::ofstream out("test_config_muninn.toml");
        out << R"(
...
use_tls = true
    ...
    void TearDown() override { std::filesystem::remove("test_config_muninn.toml"); }
};

TEST_F(ConfigTest, ParsesAeronConfig) {
    auto s = muninn::config::load("test_config_muninn.toml");
    ...
    EXPECT_EQ(s.muninn_status.stream_id, 1006);
}
