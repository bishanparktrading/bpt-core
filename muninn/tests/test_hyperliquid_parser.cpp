...
using namespace muninn::adapter;
using namespace muninn::mapping;
...
TEST(HyperliquidParser, UserFeesParsedAsBps) {
    ...
    EXPECT_EQ(fs.exchange_id, bifrost::protocol::ExchangeId::HYPERLIQUID);
    EXPECT_EQ(fs.instrument_id, 0u);  // 0 = exchange-wide
    EXPECT_EQ(fs.instrument_type, bifrost::protocol::InstrumentType::PERPETUAL);
