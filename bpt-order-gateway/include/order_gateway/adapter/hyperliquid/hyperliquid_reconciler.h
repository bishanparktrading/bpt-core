...
// Phantom-fill recovery for the Hyperliquid adapter.
//
// Scenario this closes: adapter sends a signed action over WS → bytes
// reach HL → HL accepts, possibly fills → the ack response does NOT
// reach us (WS drops mid-flight, 5 s post_action timeout fires). The
// adapter used to emit REJECTED to the strategy even though there may
// be an orphan fill or a resting order on HL. The strategy's view of
// position then diverges silently from the exchange.
//
...
// match the authoritative state back to the intent by
// (asset_idx, side, qty, price within 1 tick, time >= sent_ns - 1 s).
//
...
public:
    // Everything the adapter knows about an order whose post_action
    // either threw or timed out. exchange_symbol is the HL coin string
    // (e.g. "BTC", "ETH") — used for coin-side matching against
    // openOrders/userFills entries. sent_ns is the wall-clock nanosecond
    // at which we issued post_action; the user-fills match uses a
    // time-window that starts 1 s before this.
    struct Candidate {
    ...
    // The authoritative state the match resolved to. For OpenOrder, only
    // exch_oid is populated — the order is resting, strategy learns via
    // ACKED. For UserFill we additionally carry fill_price_e8 / fill_fee_e8
    // / fill_qty_e8 / fill_time_ns so the emit path can produce a FILLED
    // ExecEvent identical to what the WS userFills stream would have.
    struct MatchResult {
    ...
    // Pure match logic — exposed as a free function so unit tests can
    // drive it without starting the worker thread or mocking anything.
    ...
    // price_tick_e8 is the price quantum for the asset; a match allows
    // price drift up to ±price_tick_e8 to accommodate HL's tick rounding.
    ...
    // Returns (open_orders, user_fills) as returned by HL's /info
    // endpoint. Both are arrays; empty on failure (the caller's job to
    // log and back off — reconciler treats empty as "no match → REJECTED").
    using Poller =
        std::function<std::pair<boost::json::array, boost::json::array>()>;

    // Callback invoked exactly once per candidate when reconciliation
    // completes. Runs on the reconciler's worker thread.
    using OnTerminal =
    ...
    // Enqueue a candidate. Returns immediately — reconciliation runs on
    // the worker thread after the grace period.
