...
/// \file
/// \brief Binance REST auth — query-string HMAC-SHA256 with hex output.
///
...
/// \brief HMAC-SHA256(key, data) -> hex. Binance's signature format.
[[nodiscard]] std::string hmac_sha256_hex(std::string_view key, std::string_view data);

/// \brief Append `&timestamp=<now_ms>&signature=<hmac-hex>` to the given query string.
///
/// Callers construct the unsigned params via the action codec, then hand
/// them here right before sending the request so the timestamp is as
/// fresh as possible.
