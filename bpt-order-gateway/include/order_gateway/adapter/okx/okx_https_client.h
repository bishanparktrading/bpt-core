...
/// \file
/// \brief Synchronous HTTPS client for OKX's REST API.
///
/// Opens a fresh TLS connection per call — OKX REST is infrequent
/// (startup instrument fetch + periodic account snapshot), so the extra
/// handshake cost is not load-bearing and connection pooling would only
/// complicate failure handling.
///
/// Two call styles:
///   - `get_unsigned(path)` — public endpoints like
///     `/api/v5/public/instruments`.
///   - `get_signed(path)`   — private endpoints; attaches OK-ACCESS-*
///     headers.
///
...
public:
    ...
    /// \brief Unauthenticated GET.
    ///
    /// Testnet flag is honoured via the `x-simulated-trading: 1` header
    /// so demo-trading public instrument lookups route to the demo
    /// endpoints.
    [[nodiscard]] std::string get_unsigned(const std::string& path);

    /// \brief Authenticated GET. Fresh signature per call.
    ///
    /// Throws on transport errors; HTTP-level errors (non-2xx, OKX code
    /// != "0") are surfaced in the response body as-is for the caller
    /// to inspect.
