...
/// \file
/// \brief Common base for all order-gateway adapters — owns IO thread + reconnect loop.
...
///
/// Owns the IO thread, Boost.Asio io_context, TLS context, stop flag,
/// and connected flag. Subclasses implement connect_and_run() for one
/// connection session; the base class drives the reconnect loop.
///
/// **Lifecycle**:
...
///   - Base always sets connected_ = false after connect_and_run()
///     returns or throws.
...
public:
    ...
    /// \brief Drain pending exec events into `fn`. Called from the main poll thread.
    ...
    /// \brief Configure the disconnect-rate breaker. Call before start().
    ///
    /// Safe to leave at default (disabled) — tests and bootstrapped
    /// envs that don't want the guard rail keep behaving as before.
...
protected:
    ...
    /// Adapter IO thread pushes events here; main thread pops via
    /// drain_exec_events(). Capacity is config-driven
    /// (AdapterConfig.exec_queue_capacity) and must be a power of 2 —
    /// enforced by the queue ctor.
