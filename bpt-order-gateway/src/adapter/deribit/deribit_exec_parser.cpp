...
void DeribitExecParser::handle_subscription_event(const json::object& d, uint64_t recv_ns) {
    ...
    if (order_id == 0) {
        ygg::log::debug("[OrderGateway] DeribitExecParser: subscription unknown label={}", label);
        return;
    }
    ...
    if (ev.status == ES::ACKED) {
        ...
        if (!acked_orders_.insert(ev.order_id).second) {
            ygg::log::debug("[OrderGateway] DeribitExecParser: suppressed duplicate ACKED order_id={}", ev.order_id);
            return;
        }
    } else if (ev.status == ES::CANCELLED) {
        std::lock_guard<std::mutex> lk(mu_);
        if (!cancelled_orders_.insert(ev.order_id).second) {
            ygg::log::debug("[OrderGateway] DeribitExecParser: suppressed duplicate CANCELLED order_id={}", ev.order_id);
            return;
        }
...
void DeribitExecParser::handle_order_response(const json::object& order_obj, uint64_t recv_ns) {
    ...
    if (order_id == 0) {
        ygg::log::warn("[OrderGateway] DeribitExecParser: order response unknown label={}", label);
        return;
    }
    ...
    ygg::log::info("[OrderGateway] DeribitExecParser: order response label={} exchange_oid={} state={}",
                   label,
                   exch_oid,
                   order_state);
