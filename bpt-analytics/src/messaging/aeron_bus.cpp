...
AnalyticsBus AnalyticsAeronBus::build(std::shared_ptr<aeron::Aeron> aeron,
                                      const config::Settings& settings) {
    AnalyticsBus bus;
    bus.exec_sub = std::make_unique<ExecReportSubscriber>(
        aeron, settings.exec_report.channel, settings.exec_report.stream_id);
    bus.md_sub = std::make_unique<MdBboSubscriber>(
        aeron, settings.md_data.channel, settings.md_data.stream_id);
    bus.tox_pub = std::make_unique<ToxicityPublisher>(
