...
void ShortVolStrategy::start() {
    ...
    if (md_client_) {
        ...
        // Surtr reads these passively to compute implied vol surfaces.
...
void ShortVolStrategy::on_bbo(const bifrost::protocol::MdMarketData& tick) {
...
void ShortVolStrategy::on_trade(const bifrost::protocol::MdTrade& /*tick*/) {
    // Not used — we rely on BBO for RV estimation.
}

void ShortVolStrategy::on_vol_surface(bifrost::protocol::VolSurface& surface) {
    ...
    while (points.hasNext()) {
        ...
        leg.is_call = (pt.optionSide() == bifrost::protocol::OptionSide::CALL);
...
void ShortVolStrategy::on_exec_report(const bifrost::protocol::ExecutionReport& rpt) {
    ...
    if (status == ExecStatus::FILLED || status == ExecStatus::PARTIAL) {
        ...
        const bool is_buy = (rpt.side() == bifrost::protocol::OrderSide::BUY);
...
void ShortVolStrategy::evaluate(UnderlyingState& state, uint64_t now_ns) {
    ...
    for (auto& [inst_id, leg] : state.options) {
        ...
        if (leg.position_qty == 0.0) {
            ...
            send_option_order(state, leg, bifrost::protocol::OrderSide::SELL, qty, limit_price);
            ...
            send_option_order(state, leg, bifrost::protocol::OrderSide::BUY, qty, limit_price);
        }
    }
...
void ShortVolStrategy::rebalance_hedge(UnderlyingState& state, uint64_t /*now_ns*/) {
    ...
    if (delta_to_trade > 0.0) {
        side = bifrost::protocol::OrderSide::BUY;
        price = state.perp_ask + spread * aggress_bps_ / 10000.0;
    } else {
        side = bifrost::protocol::OrderSide::SELL;
...
void ShortVolStrategy::send_option_order(UnderlyingState& state,
    ...
    const uint64_t oid = order_mgr_->place_order(leg.instrument_id,
                                                 state.exchange_id,
                                                 side,
                                                 bifrost::protocol::OrderType::LIMIT,
                                                 bifrost::protocol::TimeInForce::IOC,
                                                 price,
                                                 qty);
    if (oid == 0)
        return;
