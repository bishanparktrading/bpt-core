...
void FundingArbStrategy::on_delta(const refdata::Instrument& /*inst*/,
    ...
    // restart Fenrir to pick up new instruments.
}

void FundingArbStrategy::on_trade(const bifrost::protocol::MdTrade& /*tick*/) {
...
void FundingArbStrategy::on_exec_report(const bifrost::protocol::ExecutionReport& rpt) {
...
void FundingArbStrategy::enter_position(ArbPair& pair, int direction, uint64_t now_ns) {
    ...
    const auto spot_side = (direction > 0) ? bifrost::protocol::OrderSide::BUY : bifrost::protocol::OrderSide::SELL;
...
void FundingArbStrategy::exit_position(ArbPair& pair, uint64_t now_ns) {
    ...
    const auto perp_side = (pair.direction > 0) ? bifrost::protocol::OrderSide::BUY    // close short
                                                : bifrost::protocol::OrderSide::SELL;  // close long
...
void FundingArbStrategy::handle_fill(ArbPair& pair,
    ...
    if (is_spot) {
        if (rpt.side() == bifrost::protocol::OrderSide::BUY)
...
void FundingArbStrategy::handle_terminal(ArbPair& pair, LegState& leg, bifrost::protocol::ExecStatus::Value status) {
    const bool is_spot = (leg.instrument_id == pair.spot.instrument_id);
    ...
    switch (pair.state) {
        case PairState::ENTERING_FIRST_LEG:
            if (status == ExecStatus::FILLED) {
                ...
                const auto perp_side =
                    (pair.direction > 0) ? bifrost::protocol::OrderSide::SELL : bifrost::protocol::OrderSide::BUY;
        ...
        case PairState::EXITING_FIRST_LEG:
            if (status == ExecStatus::FILLED) {
                ...
                const auto spot_side = (pair.direction > 0) ? bifrost::protocol::OrderSide::SELL  // close long spot
                                                            : bifrost::protocol::OrderSide::BUY;  // close short spot
                const uint64_t qty = static_cast<uint64_t>(std::abs(pair.spot_filled_qty));
