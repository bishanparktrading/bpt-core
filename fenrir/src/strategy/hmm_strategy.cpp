...
void HmmStrategy::on_delta(const refdata::Instrument& /*inst*/, bifrost::protocol::DeltaUpdateType::Value /*type*/) {}

// ── Market data ──────────────────────────────────────────────────────────────

void HmmStrategy::on_bbo(const bifrost::protocol::MdMarketData& tick) {
...
void HmmStrategy::on_trade(const bifrost::protocol::MdTrade& tick) {
    auto it = state_.find(tick.instrumentId());
    if (it == state_.end())
        return;
...
void HmmStrategy::on_order_book(const bifrost::protocol::MdOrderBook& book) {
...
void HmmStrategy::on_exec_report(const bifrost::protocol::ExecutionReport& rpt) {
