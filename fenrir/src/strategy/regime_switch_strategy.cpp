...
void RegimeSwitchStrategy::on_trade(const bifrost::protocol::MdTrade& /*tick*/) {}

// ── Order Book ──────────────────────────────────────────────────────────────

void RegimeSwitchStrategy::on_order_book(const bifrost::protocol::MdOrderBook& book) {
...
void RegimeSwitchStrategy::on_bbo(const bifrost::protocol::MdMarketData& tick) {
...
void RegimeSwitchStrategy::on_bar_close(InstrumentState& st) {
    ...
    const double hurst = compute_hurst_multi_window(st.log_returns, st.return_count);
    st.last_hurst = hurst;
...
void RegimeSwitchStrategy::on_exec_report(const bifrost::protocol::ExecutionReport& rpt) {
        ...
        if (order_id == st.momentum_order_id) {
            if (status == ExecStatus::FILLED) {
                if (st.has_momentum_position) {
                    ...
                    if (st.momentum_side == bifrost::protocol::OrderSide::BUY) {
...
double RegimeSwitchStrategy::compute_hurst(const std::array<double, kMaxHurstWindow>& returns, size_t count) const {
    const size_t n = std::min(count, hurst_window_);
    if (n < 20)
        return 0.5;
...
// Multi-window Hurst: compute R/S at multiple sub-window sizes and regress
// log(R/S) vs log(n). The slope is the Hurst exponent. Much more robust than
// the single-window estimator.
double RegimeSwitchStrategy::compute_hurst_multi_window(const std::array<double, kMaxHurstWindow>& returns,
                                                        size_t count) const {
    const size_t n = std::min(count, hurst_window_);
    if (n < 20)
        return 0.5;

    // Sub-window sizes: powers of 2 from 8 up to n/2.
    // Each size produces one (log_n, log_rs) data point for regression.
    double sum_x = 0.0, sum_y = 0.0, sum_xx = 0.0, sum_xy = 0.0;
    int num_points = 0;

    for (size_t win = 8; win <= n / 2; win *= 2) {
        const size_t num_chunks = n / win;
        if (num_chunks == 0)
            continue;
        ...
        for (size_t c = 0; c < num_chunks; ++c) {
            ...
            // Cumulative deviations and std dev.
    ...
    if (num_points < 2) {
        // Fall back to single-window if not enough data points for regression.
        return compute_hurst(returns, count);
    }
...
void RegimeSwitchStrategy::grid_on_fill(InstrumentState& st,
                                        uint64_t order_id,
                                        const bifrost::protocol::ExecutionReport& rpt) {
...
uint64_t RegimeSwitchStrategy::send_order(InstrumentState& st,
                                          ...
                                          bifrost::protocol::OrderType::Value type,
