...
const prometheus::Histogram::BucketBoundaries& FenrirMetrics::kDefaultLatencyBucketsNs() {
...
FenrirMetrics::FenrirMetrics(int port) {
    ...
    auto& h = prometheus::BuildGauge()
                  .Name("fenrir_healthy")
                  .Help("1 if Fenrir is running")
                  .Register(*registry);
    healthy = &h.Add({});
    healthy->Set(1.0);

    auto& sa = prometheus::BuildGauge()
                   .Name("fenrir_strategy_active")
