...
//
// Quantities are stored in 1e5 fixed-point (same scale as NewOrder.quantity).
// Prices are stored as double converted from 1e8 fixed-point.
class PositionTracker {
public:
    struct Position {
        int64_t net_qty{0};        // positive = long, negative = short (1e5 fixed-point)
    ...
    // filled_qty : 1e5 fixed-point (rpt.filledQty())
    ...
    // Returns net_qty in 1e5 fixed-point (0 if no entry).
