...
#include "fenrir/strategy/realized_vol_estimator.h"
...
//
// Data flow:
//   - Reads VolSurface from Surtr (stream 4001) for IV, Greeks, option BBO.
...
private:
    ...
    void send_perp_order(UnderlyingState& state, bifrost::protocol::OrderSide::Value side, double qty, double price);
    ...
    // Keyed by "exchange_id:underlying" (same key as Surtr's grid)
    ...
    static std::string state_key(bifrost::protocol::ExchangeId::Value ex, const std::string& underlying);
};
