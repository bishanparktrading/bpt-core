...
// Prometheus metrics exposed by Fenrir on the metrics_port from config.
// Mirrors the shape of HeimdallMetrics / HuginnMetrics for consistency
...
struct FenrirMetrics {
    ...
    // Throughput counters
    prometheus::Counter* md_ticks_total{};      // every MD tick from huginn
    prometheus::Counter* exec_reports_total{};  // every ExecReport from heimdall
