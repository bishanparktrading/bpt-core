...
namespace fenrir::backtest {

// Subscribes to BacktestControl (stream 9002, Jormungandr → Fenrir) and publishes
// BacktestAck (stream 9001, Fenrir → Jormungandr).
//
...
public:
    ...
    std::function<void(bifrost::protocol::BacktestCommand::Value cmd, uint64_t tick_seq, uint64_t simulation_ts)>
        on_control;
