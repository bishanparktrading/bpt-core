...
public:
    ...
    using OnBboFn = std::function<void(const bifrost::protocol::MdMarketData&)>;
    using OnTradeFn = std::function<void(const bifrost::protocol::MdTrade&)>;
    using OnOrderBookFn = std::function<void(const bifrost::protocol::MdOrderBook&)>;
    ...
    // Used by FenrirApp to track local receipt time for the liveness watchdog.
