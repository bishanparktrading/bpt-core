...
public:
    ...
    // Fenrir should validate against its configured_exchanges mask and halt if any expected
    // exchange is absent.
    std::function<
        void(uint8_t exchanges_loaded, uint16_t instrument_count, bool fee_schedules_loaded, bool funding_rates_loaded)>
        on_ready;

    // Fired when the refdata service reports a runtime error Fenrir must act on.
    std::function<void(bifrost::protocol::RefDataErrorType::Value error_type,
