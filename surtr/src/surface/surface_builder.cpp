...
std::vector<VolSurfaceGrid> SurfaceBuilder::build(uint32_t current_date) {
    ...
    for (const auto& [id, inst] : instruments_) {
        ...
        const bool inverse_quoted = (inst.exchange_id == bifrost::protocol::ExchangeId::DERIBIT);
        ...
        grid.points.push_back(IvPoint{
            ...
            .option_side = inst.is_call ? bifrost::protocol::OptionSide::CALL : bifrost::protocol::OptionSide::PUT,
...
std::string SurfaceBuilder::curve_key(bifrost::protocol::ExchangeId::Value ex, const std::string& underlying) {
