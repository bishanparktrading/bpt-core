...
static bifrost::protocol::FeeCurrency::Value parse_fee_ccy(const std::string& ccy) {
    ...
    if (ccy == "BTC") return FC::BTC;
    if (ccy == "ETH") return FC::ETH;
    if (ccy == "USDT") return FC::USDT;
    if (ccy == "USD") return FC::USD;
    return FC::USDT;
}
...
void OKXExecParser::handle_order_ack(const json::object& d, uint64_t recv_ns) {
    ...
    std::string cloid;
    if (auto cit = d.find("clOrdId"); cit != d.end()) cloid = std::string(cit->value().as_string());
    ...
    if (order_id == 0) {
        spdlog::warn("[Heimdall] OKXExecParser: op=order response unknown cloid={}", cloid);
        return;
    }

    std::string s_code;
    if (auto sc = d.find("sCode"); sc != d.end()) s_code = std::string(sc->value().as_string());
    std::string s_msg;
    if (auto sm = d.find("sMsg"); sm != d.end()) s_msg = std::string(sm->value().as_string());

    if (s_code == "0") {
        uint64_t exch_oid = 0;
        if (auto oi = d.find("ordId"); oi != d.end()) {
            std::string oid_str(oi->value().as_string());
            if (!oid_str.empty()) exch_oid = static_cast<uint64_t>(std::stoull(oid_str));
        }
        spdlog::info("[Heimdall] OKXExecParser: order acked cloid={} ordId={}", cloid, exch_oid);
        // ACKED events come from the orders channel — skip emitting here.
    } else {
        spdlog::error("[Heimdall] OKXExecParser: order rejected cloid={} sCode={} sMsg={}", cloid,
                      s_code, s_msg);
        ExecEvent ev{};
        ev.order_id = order_id;
        ev.exchange_id = bifrost::protocol::ExchangeId::OKX;
        ev.instrument_id = 0;
        ev.local_ts_ns = recv_ns;
        ev.exchange_ts_ns = recv_ns;
        ev.side = bifrost::protocol::OrderSide::BUY;  // placeholder; strategy knows side
...
void OKXExecParser::handle_orders_channel_item(const json::object& d, uint64_t recv_ns) {
    ...
    if (order_id == 0) {
        spdlog::warn("[Heimdall] OKXExecParser: unknown cloid={}", cloid);
        return;
    }
    ...
    if (auto tsit = d.find("uTime"); tsit != d.end())
        ev.exchange_ts_ns =
            static_cast<uint64_t>(std::stoull(std::string(tsit->value().as_string()))) * 1000000ULL;
    else
        ev.exchange_ts_ns = recv_ns;
    ...
    if (ev.status == ES::ACKED) {
        ...
        if (!acked_orders_.insert(ev.order_id).second) {
            spdlog::debug("[Heimdall] OKXExecParser: suppressed duplicate ACKED order_id={}",
                          ev.order_id);
            return;
        }
        ...
        if (!cancelled_orders_.insert(ev.order_id).second) {
            spdlog::debug("[Heimdall] OKXExecParser: suppressed duplicate CANCELLED order_id={}",
                          ev.order_id);
            return;
        }
