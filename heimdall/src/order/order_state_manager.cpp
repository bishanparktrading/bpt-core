...
static bool is_open(OrderLifecycle lc) {
    return lc == OrderLifecycle::PENDING || lc == OrderLifecycle::ACKED ||
           lc == OrderLifecycle::PARTIALLY_FILLED;
}

bool OrderStateManager::insert(const OrderState& state) {
    auto [it, inserted] = orders_.emplace(state.order_id, state);
    if (inserted && is_open(state.lifecycle)) ++total_open_;
    return inserted;
}
...
bool OrderStateManager::update(uint64_t order_id, OrderLifecycle new_lifecycle,
                               uint64_t exchange_order_id, uint64_t filled_qty,
                               uint64_t remaining_qty, uint64_t timestamp_ns) {
    auto it = orders_.find(order_id);
    ...
    if (was_open && !now_open)
        ...
        ++total_open_;
    if (exchange_order_id != 0) state.exchange_order_id = exchange_order_id;
...
void OrderStateManager::remove(uint64_t order_id) {
    ...
    if (is_open(it->second.lifecycle)) --total_open_;
    orders_.erase(it);
}

uint32_t OrderStateManager::open_order_count(bifrost::protocol::ExchangeId::Value exchange) const {
    uint32_t count = 0;
    for (const auto& [id, state] : orders_) {
        if (state.exchange_id == exchange &&
            (state.lifecycle == OrderLifecycle::PENDING ||
