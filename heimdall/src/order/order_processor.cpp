...
namespace {
inline uint64_t now_ns() noexcept { return ygg::util::TscClock::now_epoch_ns(); }
}  // namespace

namespace heimdall::order {

OrderProcessor::OrderProcessor(messaging::ExecReportPublisher& exec_pub,
                               OrderStateManager& state_mgr, risk::RiskChecker& risk_checker,
                               metrics::HeimdallMetrics& metrics,
                               const std::vector<std::shared_ptr<adapter::IOrderAdapter>>& adapters)
    : exec_pub_(exec_pub), state_mgr_(state_mgr), risk_checker_(risk_checker), metrics_(metrics) {
    adapter_by_id_.fill(nullptr);
    for (const auto& a : adapters) {
        auto idx = static_cast<uint8_t>(a->exchange_id());
        if (idx < adapter_by_id_.size()) adapter_by_id_[idx] = a.get();
    }
...
void OrderProcessor::on_exec_event(const adapter::ExecEvent& ev) {
    ...
    state_mgr_.update(ev.order_id, new_lc, ev.exchange_order_id, ev.filled_qty, ev.remaining_qty,
                      ev.local_ts_ns);
    ...
    const bool terminal =
        (new_lc == OL::FILLED || new_lc == OL::CANCELLED || new_lc == OL::REJECTED);
    if (terminal) risk_checker_.on_order_closed(ev.exchange_id);

    exec_pub_.publish(ev.order_id, ev.exchange_order_id, ev.exchange_id, ev.instrument_id,
                      ev.status, ev.side, ev.order_type, ev.price, ev.filled_qty, ev.remaining_qty,
                      ev.reject_reason, ev.fee, ev.fee_currency, ev.exchange_ts_ns, ev.local_ts_ns);
    ...
    if (new_lc == OL::ACKED) {
        if (const auto* st = state_mgr_.get(ev.order_id)) {
            if (ev.local_ts_ns > st->created_ns)
                metrics_.order_ack_rtt(exch).Observe(
                    static_cast<double>(ev.local_ts_ns - st->created_ns));
        }
    }

    if (terminal) state_mgr_.remove(ev.order_id);
}
...
void OrderProcessor::on_new_order(const bifrost::protocol::NewOrder& order) {
    ...
    spdlog::debug("[Heimdall] NewOrder: id={} exchange={} instrument_id={} qty={}", order.orderId(),
                  static_cast<int>(order.exchangeId()), order.instrumentId(), order.quantity());
    ...
    auto result = risk_checker_.check(order.exchangeId(), order.instrumentId(), order.price(),
                                      order.quantity(), order.orderId());
    if (!result) {
        const uint64_t ts = now_ns();
        exec_pub_.publish(order.orderId(), 0, order.exchangeId(), order.instrumentId(),
                          ES::REJECTED, order.side(), order.orderType(), order.price(), 0,
                          order.quantity(), result.error(), 0, FC::USDT, ts, ts);
        spdlog::warn("[Heimdall] Order {} rejected by risk: reason={}", order.orderId(),
    ...
    if (!adapter || !adapter->is_connected()) {
        ...
        exec_pub_.publish(order.orderId(), 0, order.exchangeId(), order.instrumentId(),
                          ES::REJECTED, order.side(), order.orderType(), order.price(), 0,
                          order.quantity(), RR::EXCHANGE_ERROR, 0, FC::USDT, ts, ts);
        spdlog::warn("[Heimdall] Order {} rejected: adapter not connected", order.orderId());
...
void OrderProcessor::on_cancel(const bifrost::protocol::CancelOrder& cancel) {
    spdlog::debug("[Heimdall] CancelOrder: id={} exchange={}", cancel.orderId(),
    ...
    if (!st) {
        spdlog::warn("[Heimdall] CancelOrder {}: order not found in state", cancel.orderId());
        return;
    }
...
void OrderProcessor::on_cancel_all(const bifrost::protocol::CancelAll& msg) {
    ...
    spdlog::debug("[Heimdall] CancelAll: exchange={} instrument_id={}",
                  static_cast<int>(msg.exchangeId()), msg.instrumentId());
    ...
    if (msg.exchangeId() == EX::ALL) {
        ...
        spdlog::warn("[Heimdall] Kill switch activated — cancelling all open orders");
        state_mgr_.for_each_open([&](OrderState& st) {
            auto* adapter = find_adapter(st.exchange_id);
            if (adapter) adapter->send_cancel_all(st.instrument_id);
        });
    } else {
        auto* adapter = find_adapter(msg.exchangeId());
        if (adapter) adapter->send_cancel_all(msg.instrumentId());
    }
}

void OrderProcessor::on_modify(const bifrost::protocol::ModifyOrder& modify) {
    spdlog::debug("[Heimdall] ModifyOrder: id={} exchange={}", modify.orderId(),
    ...
    if (!st) {
        spdlog::warn("[Heimdall] ModifyOrder {}: order not found in state", modify.orderId());
        return;
    }
...
void OrderProcessor::check_stale_orders(uint64_t stale_timeout_ns) {
    ...
    state_mgr_.check_stale(cur_ns, stale_timeout_ns, [&](const OrderState& st) {
        spdlog::warn("[Heimdall] Stale order detected: id={} exchange={} age_ms={}", st.order_id,
                     static_cast<int>(st.exchange_id), (cur_ns - st.last_update_ns) / 1'000'000ULL);
        ...
        exec_pub_.publish(st.order_id, st.exchange_order_id, st.exchange_id, st.instrument_id,
                          bifrost::protocol::ExecStatus::CANCELLED, st.side, st.order_type,
                          st.price, st.filled_qty, st.remaining_qty,
                          bifrost::protocol::RejectReason::OK, 0,
                          bifrost::protocol::FeeCurrency::USDT, cur_ns, cur_ns);

        stale_ids_scratch_.push_back(st.order_id);
    });

    for (uint64_t id : stale_ids_scratch_) state_mgr_.remove(id);
}

// ----- private helpers -------------------------------------------------------

adapter::IOrderAdapter* OrderProcessor::find_adapter(
    bifrost::protocol::ExchangeId::Value id) const {
    auto idx = static_cast<uint8_t>(id);
    if (idx >= adapter_by_id_.size()) return nullptr;
    return adapter_by_id_[idx];
}

OrderLifecycle OrderProcessor::exec_status_to_lifecycle(
