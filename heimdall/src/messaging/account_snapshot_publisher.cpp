...
void AccountSnapshotPublisher::publish(const adapter::AccountSnapshotData& snapshot) {
    ...
    constexpr std::size_t kBufSize = MessageHeader::encodedLength() +
                                     AccountSnapshot::sbeBlockLength() + 4 +  // groupSizeEncoding
    ...
    const auto encoded_len = static_cast<aeron::util::index_t>(
        MessageHeader::encodedLength() + msg.encodedLength());
    aeron::AtomicBuffer ab(reinterpret_cast<uint8_t*>(buf), encoded_len);

    std::lock_guard lock(mutex_);
    while (publication_->offer(ab, 0, encoded_len) < 0) std::this_thread::yield();

    spdlog::info("[Heimdall] AccountSnapshot published exchange={} balance={:.2f} positions={}",
                 ExchangeId::c_str(snapshot.exchange_id),
                 static_cast<double>(snapshot.available_balance_e8) / 1e8, n);
}
