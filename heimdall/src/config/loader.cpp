...
Settings load(const std::string& path) {
    ...
    if (auto v = root["exchange_config"].value<std::string>()) {
        ...
        if (auto* arr = exchange["adapters"].as_array()) adapters_arr = *arr;
    ...
    if (auto v = root["environment"].value<std::string>()) s.environment = *v;
    ...
    if (!s.environment.empty() && !exchange_config_path.empty()) {
        ...
        if ((s.environment == "prod" && path_has_testnet) ||
            ...
            spdlog::warn(
                "environment = \"{}\" but exchange_config = \"{}\" — possible misconfiguration",
                s.environment, exchange_config_path);
    }
    ...
    if (auto* arr = root["exchanges"].as_array()) {
        for (auto& elem : *arr)
            if (auto v = elem.value<std::string>()) exchange_filter.insert(*v);
    ...
    if (auto* aeron = root["aeron"].as_table()) {
        ...
        s.aeron.account_snapshot =
            load_stream((*aeron)["account_snapshot"].as_table(), "aeron:ipc", 3004);
    }
    ...
    for (auto& elem : adapters_arr) {
        ...
        if (!exchange_filter.count(exchange_name)) continue;

        AdapterConfig ac;
        ac.exchange = exchange_name;
        if (auto v = (*a)["secret_name"].value<std::string>()) ac.secret_name = *v;
        if (auto v = (*a)["testnet"].value<bool>()) ac.testnet = *v;
        if (auto v = (*a)["rest_host"].value<std::string>()) ac.rest_host = *v;
        if (auto v = (*a)["rest_port"].value<std::string>()) ac.rest_port = *v;
        if (auto v = (*a)["ws_host"].value<std::string>()) ac.ws_host = *v;
        if (auto v = (*a)["ws_port"].value<std::string>()) ac.ws_port = *v;
        if (auto v = (*a)["ws_path"].value<std::string>()) ac.ws_path = *v;
        if (auto v = (*a)["use_tls"].value<bool>()) ac.use_tls = *v;
