...
AeronBacktestControlPublisher::AeronBacktestControlPublisher(std::shared_ptr<aeron::Publication> pub)
    : pub_(std::move(pub)) {}

void AeronBacktestControlPublisher::send(bpt::messages::BacktestCommand::Value cmd,
    ...
    while (true) {
        ...
        if (result == aeron::BACK_PRESSURED || result == aeron::NOT_CONNECTED || result == aeron::ADMIN_ACTION) {
            if (std::chrono::steady_clock::now() > deadline)
                throw std::runtime_error("[AeronBacktestControlPublisher] offer timed out (back-pressure/not-connected)");
            std::this_thread::sleep_for(100us);
            continue;
        }

        throw std::runtime_error("[AeronBacktestControlPublisher] offer failed: " + std::to_string(result));
    }
}
