...
StrategyBus StrategyAeronBus::build(std::shared_ptr<aeron::Aeron> aeron,
    ...
    if (ac.order.stream_id != 0) {
        bus.order_gw = std::make_unique<order::AeronOrderGatewayClient>(
            aeron,
            ac.order.channel,
            ac.order.stream_id,
    ...
    if (ac.vol_surface.stream_id != 0) {
        bus.vol = std::make_unique<vol::VolSurfaceClient>(
            aeron,
    ...
    if (ac.toxicity.stream_id != 0) {
        bus.tox = std::make_unique<ToxicitySubscriber>(
            aeron, ac.toxicity.channel, ac.toxicity.stream_id);
    ...
    if (cfg.backtest_mode) {
        bus.backtest = std::make_unique<backtest::BacktestClient>(
            aeron,
    ...
    if (!cfg.backtest_mode && ac.dashboard_control.stream_id != 0) {
        bus.dashboard_ctrl = std::make_unique<DashboardControlSubscriber>(
            aeron, ac.dashboard_control.channel, ac.dashboard_control.stream_id);
    ...
    if (!cfg.backtest_mode) {
        bus.portfolio_snap = std::make_unique<dashboard::PortfolioSnapshotPublisher>(
