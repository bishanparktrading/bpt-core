...
static fs::path write_toml(const std::string& content) {
    auto path = fs::temp_directory_path() / "jormungandr_test_config.toml";
    std::ofstream f(path);
    f << content;
    return path;
}

TEST(JormungandrConfigTest, ParsesFullConfig) {
...
TEST(JormungandrConfigTest, DefaultsAppliedWhenFieldsOmitted) {
    auto path = write_toml(R"(
[simulation]
start = "2026-01-01T00:00:00Z"
end   = "2026-01-31T23:59:59Z"
...
TEST(JormungandrConfigTest, MissingFileThrows) {
    EXPECT_THROW(jormungandr::config::load("/nonexistent/jormungandr.toml"), std::exception);
}
