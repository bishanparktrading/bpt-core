...
ResultsCollector::ResultsCollector(double starting_capital, std::string output_dir)
    : starting_capital_(starting_capital), output_dir_(std::move(output_dir)) {
...
double ResultsCollector::apply_fill(Position& pos, double fill_qty, double fill_price,
                                    OrderSide side) {
    double realized = 0.0;
...
double ResultsCollector::total_realized_pnl() const {
    double sum = 0.0;
    for (const auto& [k, p] : positions_) sum += p.realized_pnl;
    return sum;
}
...
double ResultsCollector::total_unrealized_pnl() const {
    ...
    for (const auto& [k, p] : positions_) {
        if (std::abs(p.net_qty) < 1e-12) continue;
        auto mid_it = mid_prices_.find(k);
        if (mid_it == mid_prices_.end()) continue;
...
void ResultsCollector::on_fill(const matching::FillReport& fill) {
    ...
    double realized =
        apply_fill(positions_[key], fill.last_fill_qty, fill.last_fill_price, fill.side);
...
void ResultsCollector::on_market_event(const data::MarketEvent& event) {
    if (event.type != data::MarketEvent::Type::ORDER_BOOK) return;
    ...
    if (bid > 0.0 && ask > 0.0) mid_prices_[key] = (bid + ask) * 0.5;
}
...
double ResultsCollector::compute_sharpe() const {
    ...
    for (std::size_t i = 1; i < equity_curve_.size(); ++i) {
        ...
        if (prev > 1e-12) returns.push_back((equity_curve_[i].equity - prev) / prev);
    }
...
void ResultsCollector::write() const {
    ...
    // ── trades.csv ────────────────────────────────────────────────────────
    {
        ...
        if (!f) throw std::runtime_error("Cannot open " + output_dir_ + "/trades.csv");
        ...
        for (const auto& r : trades_) {
            f << std::format("{},{},{},{},{},{},{},{:.10g},{:.10g},{:.10g},{:.10g}\n",
                             r.simulation_ts, r.exchange, r.symbol, r.order_id, r.client_order_id,
                             r.side, r.order_type, r.qty, r.price, r.realized_pnl, r.equity);
        }
        spdlog::info("[ResultsCollector] Wrote {}/trades.csv ({} rows)", output_dir_,
                     trades_.size());
    }
    ...
    // ── pnl_curve.csv ─────────────────────────────────────────────────────
    {
        ...
        if (!f) throw std::runtime_error("Cannot open " + output_dir_ + "/pnl_curve.csv");
        f << "simulation_ts,equity\n";
        for (const auto& pt : equity_curve_)
            f << std::format("{},{:.10g}\n", pt.simulation_ts, pt.equity);
        spdlog::info("[ResultsCollector] Wrote {}/pnl_curve.csv ({} points)", output_dir_,
                     equity_curve_.size());
    }
    ...
    {
        double final_equity =
            equity_curve_.empty() ? starting_capital_ : equity_curve_.back().equity;
        double total_pnl = final_equity - starting_capital_;
        double return_pct =
            (starting_capital_ > 1e-12) ? (total_pnl / starting_capital_) * 100.0 : 0.0;
        double max_drawdown = compute_max_drawdown();
        double sharpe = compute_sharpe();

        int win_trades = 0;
        for (const auto& r : trades_)
            if (r.realized_pnl > 0.0) ++win_trades;
        double win_rate =
            trades_.empty() ? 0.0 : static_cast<double>(win_trades) / trades_.size() * 100.0;
        ...
        if (!f) throw std::runtime_error("Cannot open " + output_dir_ + "/summary.json");
        f << json::serialize(obj) << '\n';
        spdlog::info("[ResultsCollector] Wrote {}/summary.json", output_dir_);
        spdlog::info(
            "[ResultsCollector] Total PnL: {:.2f}  Return: {:.2f}%  "
            "MaxDD: {:.2f}%  Fills: {}  WinRate: {:.1f}%",
            total_pnl, return_pct, max_drawdown * 100.0, trades_.size(), win_rate);
    }
}
