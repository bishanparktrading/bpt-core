...
static std::string format_depth(const data::OrderBookRecord& ob, uint64_t update_id,
...
public:
    ...
    bool is_subscribed_trade(const std::string& sym_lower) const {
        return subs_.count(sym_lower + "@trade") > 0;
    }
...
private:
    void on_accept(beast::error_code ec) {
        if (ec) {
            spdlog::warn("[BinanceMdServer] accept error: {}", ec.message());
            closed_ = true;
            return;
        }
        do_read();
    ...
    void on_read(beast::error_code ec) {
        ...
        try {
            ...
            if (method == "SUBSCRIBE") {
                ...
                spdlog::debug("[BinanceMdServer] subscribed: {} streams", subs_.size());
            ...
            spdlog::warn("[BinanceMdServer] parse error: {}", e.what());
        }
...
BinanceMdServer::~BinanceMdServer() { stop(); }
...
void BinanceMdServer::start() {
    ...
    acceptor_.bind(ep);
    acceptor_.listen();
    spdlog::info("[BinanceMdServer] Listening on port {}", port_);
    do_accept();
...
void BinanceMdServer::do_accept() {
    acceptor_.async_accept([this](beast::error_code ec, tcp::socket socket) {
        if (!ec) {
            spdlog::info("[BinanceMdServer] New connection");
    ...
    net::post(ioc_, [this, event]() {
        ...
        if (event.type == data::MarketEvent::Type::TRADE) {
            ...
            for (const auto& s : sessions_)
                if (s->is_subscribed_trade(sym)) s->send(msg);
