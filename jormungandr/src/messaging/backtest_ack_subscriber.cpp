...
#include <bifrost_protocol/BacktestAck.h>
...
bool BacktestAckSubscriber::wait_for(uint64_t expected_seq, std::chrono::milliseconds timeout) {
    ...
    auto handler = [&](const aeron::concurrent::AtomicBuffer& buf, aeron::index_t offset,
                       aeron::index_t length, const aeron::Header& /*hdr*/) {
        if (found) return;  // already satisfied — drain remaining fragments

        // Decode the SBE message header to verify template ID.
        if (static_cast<std::size_t>(length) <
            MessageHeader::encodedLength() + BacktestAck::sbeBlockLength())
            return;

        char* raw = reinterpret_cast<char*>(const_cast<uint8_t*>(buf.buffer())) + offset;
        MessageHeader hdr(raw, static_cast<uint64_t>(length));
        if (hdr.templateId() != BacktestAck::sbeTemplateId()) return;

        BacktestAck ack;
        ack.wrapForDecode(raw, MessageHeader::encodedLength(), hdr.blockLength(), hdr.version(),
                          static_cast<uint64_t>(length));

        if (ack.tickSeqNum() == expected_seq) found = true;
    };

    while (!found && std::chrono::steady_clock::now() < deadline) {
        int fragments = sub_->poll(handler, 10);
        if (fragments == 0) std::this_thread::sleep_for(10us);
    }
