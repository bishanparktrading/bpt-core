...
JormungandrApp::JormungandrApp(config::Settings settings) : settings_(std::move(settings)) {
    ygg::log::info("[Jormungandr] Initialising — window: {} → {}, {} instrument(s)",
    ...
    // ── Aeron (tick-gating) ────────────────────────────────────────────────
    {
        ...
        ctx.errorHandler([](const std::exception& e) { ygg::log::error("[Jormungandr][Aeron] {}", e.what()); });
        aeron_ = ::aeron::Aeron::connect(ctx);
        ygg::log::info("[Jormungandr] Connected to Aeron MediaDriver");
    }
    ...
    ygg::log::info("[Jormungandr] Backtest tick-gating ready: ctrl_pub=stream:{} ack_sub=stream:{}",
    ...
    matching_engine_->set_fill_callback([this](matching::FillReport fill) {
        ...
        if (fill.exchange == "BINANCE") {
            ...
            ygg::log::warn("[JormungandrApp] No order server for exchange '{}' — fill dropped", fill.exchange);
        }
    });
    ...
    ygg::log::info("[Jormungandr] Ready — results will be written to {}", out_dir);
}

void JormungandrApp::run() {
    // Wait for at least one Huginn subscriber to connect before releasing data.
    // Without this gate, Jormungandr would exhaust all data before Huginn has
    // had a chance to connect and subscribe.
    const uint32_t timeout_s = settings_.simulation.subscriber_wait_timeout_s;
    ygg::log::info("[Jormungandr] Waiting for subscriber (timeout={}s)...", timeout_s);

    uint32_t waited = 0;
    while (okx_md_server_ && okx_md_server_->session_count() == 0) {
        std::this_thread::sleep_for(std::chrono::seconds(1));
        if (++waited >= timeout_s) {
            ygg::log::warn("[Jormungandr] No subscriber after {}s — starting anyway", timeout_s);
            break;
        }
    }

    if (okx_md_server_ && okx_md_server_->session_count() > 0)
        ygg::log::info("[Jormungandr] Subscriber connected, starting backtest");

    ygg::log::info("[Jormungandr] Starting backtest");
    clock_master_->run();
    results_->write();
    ygg::log::info("[Jormungandr] Backtest complete");
}
